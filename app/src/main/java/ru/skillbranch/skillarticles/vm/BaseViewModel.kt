...
abstract class BaseViewModel<T>(initState: T) : ViewModel() {
    ...
    /**
     * Инициализация начального состояния аргументом конструктора
     */
    @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
    ...
    /**
     * получение not null значения текущего состояния ViewModel
     */
    @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
    val currentState
        get() = state.value!!
     ...
     */
    protected fun <S> subscribeOnDataSource(source: LiveData<S>, onChanged: (newState: S, currentState: T) -> T?) {
